...
| `/groups` | Список групп тегов |
| `/newgroup`, `/delgroup` | Создать/удалить группу тегов (только админ) |
| `/joingroup`, `/leavegroup` | Вступить/выйти из группы тегов |
| `/call <группа>` | Позвать только участников указанной группы |
| `/settings` | Показать настройки чата |
| `/setheader [текст]` | Текст-приветствие зова (пусто — сбросить; админ) |
| `/setcooldown <сек>` | Кулдаун между зовами 0..86400 (админ) |
| `/setbatch <1..10>` | Упоминаний в одной пачке (админ) |
| `/ignore`, `/unignore` | Исключить/вернуть участника в зов (reply; админ) |
| `/setlang [ru\|en]` | Язык чата (без аргумента — показать текущий; меняет админ) |

### Локализация

Бот поддерживает русский и английский. Язык хранится на чат (`ChatSettings.language`) и меняется командой `/setlang ru|en` (админ). Локализованы основные пользовательские ответы: `/start`, `/help`, результаты `/call` (кулдаун, пусто, нет группы, отказ по правам). Конфиг-ответы админ-команд (`/settings`, `/callpolicy`) пока на русском — локализуются инкрементально через словари в `src/modules/i18n/messages.ts`.

### Настройки чата

`/settings` показывает текущие параметры. Меняет только администратор:

- `/setheader Сбор!` — текст перед зовом (без аргумента — сброс на стандартный);
- `/setcooldown 120` — пауза между зовами в секундах;
- `/setbatch 5` — сколько упоминаний слать в одном сообщении;
- **blacklist:** ответьте на сообщение участника командой `/ignore`, чтобы исключить его из зова (`/unignore` — вернуть).

### Группы тегов

Именованные подгруппы (команды/роли) для адресного зова:

- `/newgroup dev` — создать группу `dev` (админ); `/delgroup dev` — удалить.
- `/joingroup dev` / `/leavegroup dev` — вступить/выйти.
- `/groups` — список групп с числом участников.
- **`/call dev` — позвать только группу `dev`** (текст после имени прикрепляется: `/call dev сбор в 18:00`).
- Если первый токен `/call` не совпадает с именем группы — зовётся весь чат (токен считается частью текста).
